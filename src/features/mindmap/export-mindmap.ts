...
/**
 * Mind-map export helpers.
 *
 * markmap renders its tree into an <svg> that uses CSS variables
 * (var(--memoir-text), var(--memoir-accent), color-mix(...)) and cascade from
 * the app stylesheet. Serializing the DOM as-is loses those colors, so before
 * export we walk a deep clone and inline every computed style that matters.
 */
...
/** Serialize the live markmap SVG into a self-contained SVG document string. */
export function serializeMindmapSvg(svg: SVGSVGElement): string {
  ...
  // Strip interactive/obscure attributes that the live SVG keeps.
...
export async function serializeMindmapPng(svg: SVGSVGElement): Promise<string> {
  const svgText = serializeMindmapSvg(svg);
  const { width, height } = computeSvgBox(svg);
  const scale = 2;
...
/** Export the current mind map as PNG (raster) or SVG (vector). */
export async function exportMindmap(svg: SVGSVGElement, format: "png" | "svg") {
  const previousStatus = useAppStore.getState().status;
  try {
    const chosen = await getGateways().workspace.chooseExportFile({
      defaultPath: mindmapFileName(format),
